...
public class NodeOutput : ScriptableObject
{
	...
	{
		rect = new Rect (body.rect.x + labelRect.x, body.rect.y + labelRect.y, body.rect.width - labelRect.x, labelRect.height);
	}
	...
	public Rect GetKnob () 
	{
		...
		return new Rect (rect.x + rect.width, rect.y + (rect.height - knobSize) / 2, knobSize, knobSize);
	}
}