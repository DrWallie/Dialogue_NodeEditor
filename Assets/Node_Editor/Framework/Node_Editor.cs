...
public class Node_Editor : EditorWindow 
{
	...
	public void checkInit () 
	{
		if (!initiated || nodeCanvas == null) 
		{
            ...
            ConnectorKnob = AssetDatabase.LoadAssetAtPath(editorPath + "knob.png", typeof(Texture2D)) as Texture2D;
...
//			Node.ApplyConnection (calcNode1.Outputs [0], calcNode2.Inputs [0]);
	...
	private void InputEvents () 
	{
		...
		{ // Draw background when repainting
			Vector2 offset = new Vector2 (nodeCanvas.scrollOffset.x%Background.width - Background.width, nodeCanvas.scrollOffset.y%Background.height - Background.height);
			...
			for (int x = 0; x < tileX; x++) 
			{
				...
				{
					Rect texRect = new Rect (offset.x + x*Background.width, offset.y + y*Background.height, Background.width, Background.height);
		...
		if (e.type == EventType.MouseDown) 
		{
			...
			{ // A click on the empty canvas
				...
				{ // Right click -> Editor Context Click
                    ...
                    menu.AddItem(new GUIContent("Add Dialogue Node"), false, ContextCallback, "dialNode");
					menu.AddSeparator("");
